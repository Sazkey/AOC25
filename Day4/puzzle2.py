...
with open("puzzleInput", "r") as file:
    ...
    change = 1
    while change > 0:
        grid_copy = copy.deepcopy(this_grid)
        change = 0
        for this_i in range(this_rows):
            for this_j in range(this_cols):
                moveable = copy.deepcopy(result_papers)
                if this_grid[this_i][this_j] == "@":
                    result_papers += adjacent_paper(this_grid, this_i, this_j)
                    if moveable < result_papers:
                        grid_copy[this_i] = grid_copy[this_i][:this_j] + "." + grid_copy[this_i][this_j + 1:]
                        change += 1
